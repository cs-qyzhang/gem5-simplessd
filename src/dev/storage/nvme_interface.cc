...
using gem5::curTick;
using gem5::registerExitCallback;
...
Tick NVMeInterface::writeConfig(PacketPtr pkt) {
  ...
  if (offset < PCI_DEVICE_SPECIFIC) {
    ...
    Addr BAR0Addr = BARs[0]->addr();
    Addr BAR1Addr = BARs[1]->addr();
    Addr BAR4Addr = BARs[4]->addr();
    Addr BAR5Addr = BARs[5]->addr();
    int BAR0Size  = BARs[0]->size();
    int BAR4Size  = BARs[4]->size();
    int BAR5Size  = BARs[5]->size();

    // Updates on BAR0/1 address
    if (offset == PCI0_BASE_ADDR0 || offset == PCI0_BASE_ADDR1) {
      registerTableBaseAddress = BAR0Addr | ((uint64_t)BAR1Addr << 32);
      registerTableSize = BAR0Size;
    }
    else if (offset == PCI0_BASE_ADDR4) {
      tableBaseAddress = BAR4Addr;
      tableSize = BAR4Size;
    }
    else if (offset == PCI0_BASE_ADDR5) {
      pbaBaseAddress = BAR5Addr;
      pbaSize = BAR5Size;
    }
  }
  else {
